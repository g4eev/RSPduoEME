...
// These API routinese are derived from the sample code provided by SDRplay for
// interfacing with the sdrplay_api DLL.
...
void RSPduoInterface::Start(int LOfrequeccy, int gRdB_Gain, int LNAstate)
{
...
//        }
        ...
        if (ndev > 0)
        {
            ...
            if (chParams != NULL)
                ...
                chParams->tunerParams.gain.gRdB = gRdB_Gain;                  // set If gain reduction
...
void RSPduoInterface::ChangeIFGain(int gRdb_Gain)
{
    QString MessageString;
    sdrplay_api_ErrT err;
...
void RSPduoInterface::ChangeLNAGain(int LNAstate)
{
    QString MessageString;
    sdrplay_api_ErrT err;
    ...
    chParams->tunerParams.gain.LNAstate = LNAstate;
        // Apply limits
        if (chParams->tunerParams.gain.LNAstate > 9) chParams->tunerParams.gain.LNAstate = 9;
        if (chParams->tunerParams.gain.LNAstate < 0) chParams->tunerParams.gain.LNAstate = 0;
    if ((err = sdrplay_api_Update(chosenDevice->dev, chosenDevice->tuner,
        sdrplay_api_Update_Tuner_Gr, sdrplay_api_Update_Ext1_None)) != sdrplay_api_Success)
    {
        MessageString = QString::asprintf("Update Tuner Failed: %s", sdrplay_api_GetErrorString(err));
