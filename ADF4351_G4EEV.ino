...
//
//  Modified by G4EEV (1/10/2020)
//  Added Serial Event Read function to read, echo and set KHZ frequency from serial port at 9600 baud.
//  Changed 10MHZ to 24MHZ for external ref frequency choice.
//
//  *************************************************** ENGLISH ***********************************************************
//
...
//    for the frequency, the memories and the frequency reference (24 or 25 MHz):
...
//   - to select the refrence frequence,move the cursor on 24 or 25 and select with UP and DOWN.
...
//    appears on the screen. This memorization works then the cursor is anywhere except on the reference 24 or 25 position.
//    - For the reference frequency, move the cursor to 24 or 25, the press the SELECT for one second. 
...
void serialEvent()
{
   ...
   long frequency = Serial.parseInt(); // KHz

    // echo input if in range and set new frequency
    if((frequency > 34500)&&(frequency < 4400000))
    {
...
void setup() {
  ...
  lcd.display();
  analogWrite(10,64); //Luminosite LCD   255= max bright

  Serial.begin (9600);    //  Serial to the PC via Arduino "Serial Monitor" at 9600
  Serial.setTimeout(250); //  Serial timeout set to read 10 characters @ 9600
  ...
  switch (lcd_key)               // Select action
  {
    ...
    case btnUP: //Haut
      ...
      { 
        ...
        if (poscursor==15){ 
        if( PFDRFout==24){PFDRFout=25;} //reglage FREF
        else if ( PFDRFout==25){PFDRFout=24;}
        else PFDRFout=25;// au cas ou PFDRF different de 24 et 25
        modif=1;  }
